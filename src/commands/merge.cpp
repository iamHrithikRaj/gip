...
void printConflictHelp() {
    ...
    std::cerr << "  • " << kColorCyan << "behaviorClass" << kColorReset
              << ": Is this a feature, bugfix, or refactor?\n";
    std::cerr << "  • " << kColorCyan << "rationale" << kColorReset
              << ": Why was this change made?\n";
    std::cerr << "  • " << kColorCyan << "preconditions/postconditions" << kColorReset
              << ": Expected state before/after\n";
    std::cerr << "  • " << kColorCyan << "sideEffects" << kColorReset
...
auto merge(const std::vector<std::string>& args) -> int {
    ...
    if (result.exitCode != 0 && isMergeInProgress(git)) {
        ...
        if (result.success()) {
            std::cerr << kColorGreen << "✓ " << kColorReset << "Merge completed successfully.\n";
        }
    }
