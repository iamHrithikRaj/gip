...
1.  **Initialize**: Run `gip init` once to set up the repo.
2.  **Generate Manifest**:
    *   **Humans**: Edit `manifest.toon` manually.
    *   **Agents**: Use the diff to automatically generate the manifest. Gip provides the prompt structure.
    ```yaml
    # manifest.toon (Generated by Agent)
    schemaVersion: "2.0"
    globalIntent:
      behaviorClass: ["perf"]
...
*   **For Agents**: `gip context --export` dumps the full semantic history in TOON/JSON format, perfect for context injection before starting a task.
