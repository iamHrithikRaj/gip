...
std::vector<std::string> extractArrayFromFirstEntry(const std::string& content, const std::string& key)
{
    std::vector<std::string> result;
    std::istringstream stream(content);
    std::string line;
        ...
        if (inFirstEntry) {
            ...
            if (trimmedLine.find(key + ":") == 0 || trimmedLine.find(key + " :") == 0) {
                ...
                if (bracketStart != std::string::npos && bracketEnd != std::string::npos) {
                    // Parse inline array
                    std::string arrayContent = trimmedLine.substr(bracketStart + 1, bracketEnd - bracketStart - 1);
...
bool MergeDriver::enrichConflictMarkers(const std::string& filePath,
    ...
    while (std::getline(input, line)) {
        if (isConflictMarker(line, kConflictStart)) {
            ...
            if (theirsContext.has_value()) {
                output << formatEnrichedMarker(extractMarkerInfo(line), 
                                               "Their changes", 
                                               *theirsContext);
            }
            output << line << "\n";
...
std::optional<ConflictContext> MergeDriver::extractContext(
    ...
    const std::string& filePath) const
{
    ...
    if (context.symbol.empty()) {
        ...
        context.symbol = (lastSlash != std::string::npos) 
...
std::string MergeDriver::formatEnrichedMarker(
                                            ...
                                            const std::string& filePath) const
{
    ...
    if (matchedEntry) {
        ...
        if (context.behaviorClass.empty()) context.behaviorClass = matchedEntry->type;
        ...
        if (context.symbol.empty()) context.symbol = matchedEntry->file;
...
bool MergeDriver::writeFile(const std::string& filePath,
